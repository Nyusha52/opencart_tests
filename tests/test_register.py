...
class TestRegistrPozitive:
    ...
    @pytest.mark.parametrize('name', ['a', "a" * 32, "lat"],
                             ids=["один символ", "32 символов", "латиница"])
    def test_register_name(self, browser, name):
        ...
        data.name = name
        main_page.find_user()
        register_account._send_keys(
            element=register_account._find_element(register_account.FIRSTNAME), text=name)

        register_account._send_keys(
            element=register_account._find_element(register_account.LASTNAME),
            text=data.lastname)
        register_account._send_keys(element=register_account._find_element(register_account.EMAIL),
                                    text=data.email)
        register_account._send_keys(
            element=register_account._find_element(register_account.TELEPHONE),
            text=data.telephone)
        register_account._send_keys(
            element=register_account._find_element(register_account.PASSWORD),
            text=data.password)
        register_account._send_keys(
            element=register_account._find_element(register_account.PASSWORD_CONFIRM),
            text=data.password)
...
class TestRegistrNegative:
    data_with_one_empty_param = [
        ({'firstName': 'Имя', 'lastName': 'Фамилия', 'email': 'test@test.com',
          "telephone": "555555555",
          'password_confirm': '12345678',
          'assert': 'Password must be between 4 and 20 characters!'}),
        ({'password': '12345678', 'firstName': 'Имя', 'lastName': 'Фамилия',
          'email': 'test@test.com',
          'password_confirm': '12345678',
          'assert': 'Telephone must be between 3 and 32 characters!'}),
        ({'password': '12345678', 'lastName': 'Имя', 'email': 'test@test.com',
          "telephone": "555555555",
          'password_confirm': '12345678',
          'assert': 'First Name must be between 1 and 32 characters!'}),
        ({'password': '12345678', 'firstName': 'Имя', 'email': 'test@test.com',
          "telephone": "555555555",
          'password_confirm': '12345678',
          'assert': 'Last Name must be between 1 and 32 characters!'}),
        ({'password': '12345678', 'firstName': 'Имя', 'lastName': 'Фамилия',
          "telephone": "555555555",
          'password_confirm': '12345678',
    ...
    def test_register_one_empty_param(self, browser, data):
        ...
        if "firstName" in data:
            register_account._send_keys(
                element=register_account._find_element(register_account.FIRSTNAME),
                text=data["firstName"])
        if "lastName" in data:
            register_account._send_keys(
                element=register_account._find_element(register_account.LASTNAME),
        ...
        if "email" in data:
            register_account._send_keys(
                element=register_account._find_element(register_account.EMAIL),
                text=data["email"])
    ...
    def test_register_password_not_equals_confirm_password(self, browser):
        ...
        main_page.find_user()
        register_account._send_keys(
            element=register_account._find_element(register_account.FIRSTNAME), text=data.name)
        register_account._send_keys(
            element=register_account._find_element(register_account.LASTNAME),
            text=data.lastname)
        register_account._send_keys(element=register_account._find_element(register_account.EMAIL),
